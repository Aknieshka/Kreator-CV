function Template1() {
    ...
    var photopositionx = 0;
    var photopositiony = 0;
    ...
    if(name.length>28)
    {
        ...
        {
            doc.setFontSize(19);
            doc.text(startx, starty, name.toUpperCase()); 
            photopositiony += 15;
        }
        else
        {
            doc.setFontSize(19);
            doc.text(startx, starty, ($('#name1').val()+' '+$('#name2').val()).toUpperCase()); 
            starty+=8;
            doc.text(startx,starty,$('#name3').val().toUpperCase());
            photopositionx += 7;
        }
    }
    else {
    ...
    if(((email.length+home.length+number.length+dateOfBirth.length)*2.4)<135)
        {
            //email
    ...
    startx = 12; //POCZATEK SEKCJI EDUKACJA
    ...
    if(EducationCorrect()==true)
    {
        ...
        doc.text(start,starty,"EDUKACJA"); 
        doc.addImage(bigLine,'JPEG',start-2,starty+2,108,2);  //WZOR LINIA DLUGA
    ...
    {
        startx = 12; //POCZATEK SEKCJI DOSWIADCZENIE
        ...
        doc.addImage(bigLine,'JPEG',start-2,starty+2,108,2); 
        starty=starty+12;
    ...
    startx=125; //poczatek prawej sekcji


    if (photoFile.length > 0) //testowo
    {
        starty=58;
    }
    else {
    ...
    {
        doc.setFontSize(16);
        doc.setTextColor(naglowkicolor); //niebieski
        doc.setFont('MontserratB');

        doc.text(startx,starty,"UMIEJĘTNOŚCI"); //napis
        doc.addImage(bigLine,'JPEG',startx-2,starty+2,68,2); //linia
        starty+=9;
        startx+=2;
    ...
    if(InterestsCorrect()==true)
    {
        ...
        doc.addImage(bigLine,'JPEG',startx-2,starty+2,68,2);
...
if (photoFile.length > 0){
    ...
    fileReader.onload = function(fileLoadedEvent) {
        ...
        doc.addImage(Base64data,'PNG',148+photopositionx,5+photopositiony,35,35);
